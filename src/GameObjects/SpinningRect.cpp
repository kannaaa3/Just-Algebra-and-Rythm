...
void SpinningRect::loadMedia() {
  ...
  if (!spinningRectTexture[IDLE].loadFromFile(gRenderer, prefix + "FilledRect.png")) 
    printf("Failed to load FilledRectIDLE texture!\n");

  if (!spinningRectTexture[NORMAL].loadFromFile(gRenderer, prefix + "FilledRect.png")) 
    printf("Failed to load FilledRect texture!\n");
  
  if (!spinningRectTexture[SPLASH].loadFromFile(gRenderer, prefix + "FilledRectSplash.png")) 
    printf("Failed to load FilledRectSplash texture!\n");

  if (!spinningRectTexture[DISAPPEAR].loadFromFile(gRenderer, prefix + "FilledRect.png")) 
    printf("Failed to load FilledRect texture!\n");
...
bool SpinningRect::checkCollision(Player* p) {
  ...
  pair <float, float> topLeft = rotateAxis(rAngle,
                                           posX - width/2,
                                           -(posY - height/2));
  pair <float, float> botRight = rotateAxis(rAngle,
                                           posX + width/2,
                                           -(posY + height/2));
  ...
  pair <float, float> pLL = rotateAxis(rAngle, 
                                       (float) (p->getPosX()) - (float) p->P_SIZE * 0.5 - (float) SCREEN_WIDTH * 0.5, 
                                       (-1) * ((float) (p->getPosY()) + (float) p->P_SIZE * 0.5 - (float) SCREEN_HEIGHT * 0.5));
  pair <float, float> pUR = rotateAxis(rAngle, 
                                       (float) (p->getPosX()) + (float) p->P_SIZE * 0.5 - (float) SCREEN_WIDTH * 0.5,
                                       (-1) * ((float) (p->getPosY()) - (float) p->P_SIZE * 0.5 - (float) SCREEN_HEIGHT * 0.5));
  ...
  return SDL_PointInFRect(&pLowerLeft, &rotatedRect) | SDL_PointInFRect(&pUpperRight, &rotatedRect);
}
...
void SpinningRect::setPosition(int x, int y) { posX = x; posY = y; }
void SpinningRect::setDimension(int w, int h) {width = w; height = h;}
